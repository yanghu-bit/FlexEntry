...
class Topology(object):
    def __init__(self, config, data_dir='./data/'):
        ...
        #self.simple_paths_file = self.topology_file +'_simple_paths'
            ...
            for s in range(self.num_nodes):
                for d in range(self.num_nodes):
                    if s != d:
                        ...
                        self.pair_sd_to_idx[(s,d)] = self.num_pairs
                        self.num_pairs += 1
                        #self.preconfig_paths.append(list(nx.shortest_simple_paths(self.DG, s, d)))
                        #self.preconfig_paths.append(list(nx.shortest_simple_paths(self.DG, s, d, weight='weight')))
    ...
    def recalculate_shortest_paths(self, failure_links):
        DG = nx.DiGraph()
        print('[*] Loading topology...', self.topology_file)
        f = open(self.topology_file, 'r')
        header = f.readline()
        num_nodes = int(header[header.find(':')+2:header.find('\t')])
        num_links = int(header[header.find(':', 10)+2:])
        f.readline()
        for line in f:
            link = line.split('\t')
            i, s, d, w, c = link
            if int(i) not in failure_links:
                DG.add_weighted_edges_from([(int(s),int(d),int(w))])
        assert len(DG.nodes()) == num_nodes and len(DG.edges()) == num_links-len(failure_links)
        
        shortest_paths = []
        for s in range(num_nodes):
            for d in range(num_nodes):
                if s != d:
                    shortest_paths.append(list(nx.all_shortest_paths(DG, s, d, weight='weight')))
                    assert len(shortest_paths[-1]) > 0, (s, d)
        print('Nodes: %d, total_links: %d, failure_links: %d'%(num_nodes, num_links, len(failure_links)))
...
class Environment(object):
    def __init__(self, config, is_training=False):
        ...
        if config.topology_file == 'Telstrav2' or config.topology_file == 'Googlev2':
            self.traffic_matrices = self.traffic.traffic_matrices*100/300/1000    #kbps
        else:
        ...
        #self.simple_paths_node = self.topology.simple_paths
        #self.simple_paths_link = self.convert_to_edge_path(self.simple_paths_node)
