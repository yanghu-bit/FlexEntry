...
def sim(config, network, game, moves_queue):
    ...
    for tm_idx in game.tm_indexes:
        ...
        random_state = np.random.RandomState(seed=1024)
        ...
        if actions[0] == 0:
            ecmp_mlu, _ = game.eval_ecmp_traffic_distribution(tm_idx)
            actionsx = [-1]
        if actions[0] == 1:
        ...
        game.evaluate(tm_idx, actionsx, eval_delay=False, weighted_ecmp=True)
