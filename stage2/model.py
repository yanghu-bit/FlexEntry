...
class Network_model():
    ...
    def policy_loss_fn_action_combination(self, logits, actions, advantages, entropy_weight=0.01, log_epsilon=1e-12):
        ...
        entropy = tf.nn.softmax_cross_entropy_with_logits(labels=policy,
                                                          logits=logits)  # entropy already includes "-", shape = [batch_size,]
        entropy = tf.expand_dims(entropy, -1)  # [batch_size, 1]
        policy = tf.expand_dims(policy, -1)  # policy shape = [batch_size, action_dim, 1]
    ...
    def restore_ckpt(self, model_id, checkpoint=''):
        ...
        managerx = tf.train.CheckpointManager(self.ckpt, ckpt_dir, max_to_keep=1000)

        if checkpoint == '':
            checkpoint = managerx.latest_checkpoint
        else:
